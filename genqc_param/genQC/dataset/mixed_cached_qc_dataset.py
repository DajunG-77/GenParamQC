...
class Mixed_Cached_OpenClip_Dataset(Cached_OpenClip_Dataset):  
    ...
    def params_config(self):
        ...
        params_config["gate_pool"] = [class_to_str(gate) if not isinstance(gate, str) else gate for gate in params_config["gate_pool"]]
        # params_config["gate_pool"] = params_config["gate_pool"]
        ...
        for k,v in parameters["store_dict"].items(): 
            if k != "x" and k != "y" and k != "z":   
                ...
                if v == "tensor" and k == "U":    # hardcoded U padding !!
                    ...
                    # ci_s = torch.zeros(shape, device=c[0][add_ind].device, dtype=torch.complex64)
                    ci_s = torch.zeros(shape, device=c[0][add_ind].device)             
                  
                    run_i = 0
                    for i,ci in enumerate(c):
                        ci = ci[add_ind]   
                        ci = ci.unsqueeze(1).expand(-1, 2, -1, -1)                                              
