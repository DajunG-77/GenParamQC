...
class DiffusionPipeline(Pipeline):
        ...
        if cached_text_enc:
            ...
            def cached_empty_token_fn(c):
                if c.dim() == 1:
                    ...
                    )  # yields then a list of ints
                elif c.dim() == 2:
                    return self.empty_token.expand(c.shape)  # tokenized input
                else:
            ...
            self.empty_token_fn = lambda c: self.empty_token.expand(
                c.shape
            )  # for own clip
    ...
    def params_config(self, save_path: str):
        ...
        params_config["text_encoder"] = self.text_encoder.get_config(
            save_path=save_path + "text_encoder.pt"
        )
        ...
        # params_config["add_config"]            = {"dataset" : self.add_config["dataset"]["params"]}
    ...
    def store_pipeline(self, config_path: str, save_path: str):
        ...
        self.text_encoder.store_model(
            config_path=None, save_path=save_path + "text_encoder.pt"
        )
    ...
    def from_config_file(config_path, device: torch.device):
        ...
        config["params"]["scheduler"] = instantiate_from_config(
            config["params"]["scheduler"]
        )

        model_path = (
            config_path + "model.pt"
            if config["params"]["model"]["save_path"] is None
            else config["params"]["model"]["save_path"]
        )
        config["params"]["model"] = Config_Model.from_config(
            config["params"]["model"], device, model_path
        )

        config["params"]["text_encoder"] = Config_Model.from_config(
            config["params"]["text_encoder"],
            device,
            config["params"]["text_encoder"]["save_path"],
        )
        ...
        if exists(pipeline.add_config):
            pipeline.gate_pool = [
                gate for gate in add_config["dataset"]["params"]["gate_pool"]
            ]
    ...
    @staticmethod
    def from_config_file_initial(config_path, device: torch.device):
        config = load_config(config_path + "config.yaml")
        config = config_to_dict(config)
    ...
    def from_pretrained(cls, repo_id: str, device: torch.device, **kwargs):
        ...
        model_path = snapshot_download(
            repo_id=repo_id,
            repo_type="model",
            allow_patterns=["*.pt", "*.yaml", "*.safetensors"],
            **kwargs,
        )
    ...
    def __call__(
        ...
        x0 = self.denoising(
            latents, c=c, no_bar=no_bar, enable_guidance=enable_guidance, g=g
        )
    ...
    def latent_filling(
        ...
        **kwargs,
        ...
        if mask.dim() == 4:
            assert list(org_latents.shape) == list(
                mask.shape
            )  # diff mask per sample and channel
        elif mask.dim() == 3:
            assert list(org_latents.shape[1:]) == list(
                mask.shape
            )  # diff mask per channel
        elif mask.dim() == 2:
            assert list(org_latents.shape[2:]) == list(mask.shape)  # all same mask
        else:
            raise RuntimeError("")
        ...
        if t_start_index > 0:
            latents = self.scheduler.add_noise(
                org_latents, noise, self.scheduler.timesteps[t_start_index]
            )
        else:
            latents = noise
        ...
        for i, t in enumerate(tqdm(timesteps, disable=no_bar)):
            ...
            latents, x0 = self.denoising_step(
                latents, timesteps, c_emb=c_emb, enable_guidance=enable_guidance, g=g
            )
            ...
            if i < len(timesteps) - 1:  # else we are finished with denoising
                ...
                # noise = torch.randn(org_latents.shape, device=self.device)
                noisy_org_latents = self.scheduler.add_noise(
                    noisy_org_latents, noise, noise_timestep
                )
        ...
        if return_predicted_x0:
            return latents.cpu(), predicted_x0
        return latents.cpu()
    ...
    def denoising(
        ...
        **kwargs,
        ...
        for i, t in enumerate(
            tqdm(self.scheduler.timesteps[t_start_index:], disable=no_bar)
        ):
            timesteps = (
                (torch.ones((1)) * t)
                .type(torch.int64)
    ...
    def CFG(self, eps_u, eps_c, g):
        ...
        if (
            self.guidance_sample_mode == "normal"
        ):  # from https://arxiv.org/pdf/2207.12598.pdf, w=g+1
            eps = eps_u + g * (eps_c - eps_u)

        elif self.guidance_sample_mode == "fastai":  # from fastAi less 11
            eps = eps_u + g * (eps_c - eps_u) * torch.linalg.vector_norm(
                eps_u, dim=dim, keepdim=True
            ) / torch.linalg.vector_norm(eps_c - eps_u, dim=dim, keepdim=True)
            eps = (
                eps
                * torch.linalg.vector_norm(eps_u, dim=dim, keepdim=True)
                / torch.linalg.vector_norm(eps, dim=dim, keepdim=True)
            )

        elif (
            self.guidance_sample_mode == "rescaled"
        ):  # from https://arxiv.org/pdf/2305.08891.pd
            phi = 0.7

            eps_cfg = eps_u + g * (eps_c - eps_u)
            eps_scaled = (
                eps_cfg
                * eps_c.std(dim=dim, keepdim=True)
                / eps_cfg.std(dim=dim, keepdim=True)
            )
            eps = phi * eps_scaled + (1.0 - phi) * eps_cfg

        else:
            raise NotImplementedError(
                "Use as guidance_sample_mode one of: 'normal', 'fastai' or 'rescaled'"
            )
    ...
    def train_step(self, data, **kwargs):
        ...
        if self.enable_guidance_train:
            ...
            rnd = (rnd > self.guidance_train_p).type(
                torch.int64
            )  # todo: change to bernoulli dist fn
            rnd = self.scheduler.unsqueeze_vector_to_shape(
                rnd, y.shape
            )  # e.g. [b, 1, 1]
