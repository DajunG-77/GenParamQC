...
def instruction_name_to_qiskit_gate(name: str) -> Gate:
    ...
    return get_obj_from_str(f"qiskit.circuit.library.{name}Gate")
...
def rnd_circuit(num_of_qubits, num_of_gates, gate_pool: list[Gate], rng):
    ...
    for gate_index in gate_indices:
        ...
        if gate_qiskit_class == ql.MSGate:
            # params = {"num_qubits": int(2), "theta": rng.uniform(low=0, high=2*np.pi, size=1)}
            gate = gate_qiskit_class(2, rng.uniform(low=0, high=2*np.pi, size=1)[0]) 
        else:
        ...
        # if gate_qiskit_class == qi.MSGate and num_of_qubits < 2:
        #     change_indice = gate_index
        #     while change_indice == gate_index:
        #         change_indice = rng.choice(len(gate_pool), 1)[0]
            
        #     gate_qiskit_class = gate_pool[change_indice]
        #     num_of_paramters = get_number_of_gate_params(gate_qiskit_class)
        ...
        # elif gate_qiskit_class == qi.MSGate:
