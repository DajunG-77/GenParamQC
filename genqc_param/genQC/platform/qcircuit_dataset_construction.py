...
def encode_circuit(qc: QuantumCircuit, num_of_qubits, gate_classes: dict, max_gates: int, sign_labels={"control_qubits":-1, "target_qubits":+1}, return_params=False):
    ...
    if return_params: 
        ...
        params_tensor     = torch.zeros((num_of_qubits, max_gates), dtype=torch.float32)
    ...
    for t in range(time):         
        ...
        for gate_index,gate_qiskit_class in gate_qiskit_classes.items():                       
            ...
            if target_qubits.nelement() > 0:
                ...
                if gate_qiskit_class == ql.MSGate:
                    # print(target_qubits)
                    # print(control_qubits)
                    num_of_paramters = 1
                    if exists(params_tensor) and num_of_paramters > 0:
                        param = params_tensor[:num_of_paramters, t].tolist()[0]
                    else:                                               
                        param = 0
                    
                    qc.append(gate_qiskit_class(2, param), [*control_qubits.tolist(), *target_qubits.tolist()], [])        
...
# %% ../../src/platform/qcircuit_dataset_construction.ipynb 18
def gen_compilation_rndGates_dataset_param(samples, num_of_qubits, min_gates, max_gates, gate_pool, min_sub_gate_pool_cnt=1, silent=False):
    '''Samples rnd circuit with a rnd subset of gates and return qc with gate label and unitary'''

    # gate_classes = {x().name:(i+1) for i,x in enumerate(gate_pool)} #+1 for empty! global gate classes so we fix the indices! 1...H   2...CX   so on
    gate_classes = gate_pool_to_gate_classes(gate_pool)
    #-------------------------------
    ...
    for i in tqdm(range(samples), disable=silent):
        ...
        # qc, qc_tensor, svr, params_tensor  = get_rnd_encoded_circuit(num_of_qubits, min_gates, max_gates, sub_gate_pool, gate_classes, rng, optimized=True, return_params=True)
        qc, qc_tensor, svr, params_tensor  = get_rnd_encoded_circuit(num_of_qubits, min_gates, max_gates, gate_pool, gate_classes, rng, optimized=True, return_params=True)

        # print(qc_tensor.shape, params_tensor.shape)
        tensor.append(qc_tensor)
        params_tensors.append(params_tensor)
        # label.append(f"Compile using: {[x().name for x in gate_pool]}")
        
        # label.append(f"Compile using: {[x.__name__ for x in gate_pool]}")
        label.append(f"Compile using: {[x for x in gate_classes.keys()]}")
        # print(label)

        # qc = decode_circuit(qc_tensor, gate_pool=sub_gate_pool, params_tensor=params_tensor)
        # U.append(qi.Operator(qc).to_matrix())
        U.append(qi.Operator(qc).to_matrix())
    ...
    # make sure we have unique circuits
    tensor = torch.stack(tensor, dim=0)
    # print(params_tensors[0].shape)
    # print(params_tensors[0], params_tensors[0].size(0), params_tensors[1].size(0))
    max_len = max(tensor.size(0) for tensor in params_tensors)
    padded_tensors = [F.pad(tensor, (0, 0, 0, max_len - tensor.size(0))) for tensor in params_tensors]
    padded_tensors = torch.stack(padded_tensors, dim=0)

    U_tensor = [torch.from_numpy(u) for u in U]
    U_tensor = torch.stack(U_tensor, dim=0)
    tensor_unique, tensor_indices = dahe.get_unique_elements_indices(tensor)
    ...
    x = tensor[tensor_indices]
    para = padded_tensors[tensor_indices]
    # para = [params_tensors[i] for i in tensor_indices.tolist()]
    y = [label[i] for i in tensor_indices.tolist()]
    # U = [    U[i] for i in tensor_indices.tolist()]
    U = U_tensor[tensor_indices]
