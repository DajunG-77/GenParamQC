...
class QC_Cond_UNet(Config_Model):
    ...
    def __init__(self, model_features=[32,32,64], clr_dim=8, num_clrs=8, t_emb_size=128, cond_emb_size=512, 
        ...
        self.conv_in = nn.Conv2d(self.clr_dim + 1, model_features[0], kernel_size=1, stride=1, padding ="same") #was kernel_size=3
        ...
        self.head    = nn.Conv2d(self.dec_chs[-1], self.clr_dim + 1, kernel_size=1, stride=1, padding ="same")
    ...
    def invert_clr(self, x):
        ...
        total = x.size(1)
        x, params = x.split([total - 1, 1], dim=1)

        params = params.squeeze(1)
        params = params.to(input_device)
