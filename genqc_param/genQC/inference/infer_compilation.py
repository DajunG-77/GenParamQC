...
def get_new_unitary_indices(Us, dataset, silent=False):
    ...
    if not silent:
        print(
            f"- Checking {Us.shape[0]} unitaries for duplicates in dataset, {torch.unique(Us, dim=0).shape[0]} given unitaries are unique."
        )
    ...
    comp = Us_dataset.unsqueeze(dim=0) == Us.unsqueeze(
        dim=1
    )  # gives [num of Us, num of dataset, ch]
    ...
    if not silent:
        print(
            f"- Checked {Us.shape[0]} given unitaries with dataset. Returned indices of {comp.shape[0]} not in dataset unitaries."
        )
...
def get_new_unitary_indices_batch(
    Us, dataset, auto_batch_size=32, silent=False, n_jobs=1
):
    if type(Us) == list:
        Us = torch.stack([split_U_to_tensor(U) for U in Us])  # numpy to torch
    ...
    if n_jobs > 1:
        ...
        indices = Parallel(n_jobs=n_jobs)(
            delayed(f)(Us_chunk) for Us_chunk in Us_chunks
        )
    ...
    if not silent:
        print(
            f"- Checked {samples} given unitaries with dataset. Returned indices of {indices.shape[0]} not in dataset unitaries."
        )
...
def generate_comp_tensors(
    ...
    for batch_sample in batch_samples:
        ...
        latents = torch.randn(
            (c_batch.shape[0], pipeline.model.clr_dim + 1, system_size, max_gates)
        )
    ...
    out_tensor, params = pipeline.model.invert_clr(out_tensor)
    out_tensor = out_tensor[:, :num_of_qubits]
    params = params[:, :num_of_qubits]

    # if unique: out_tensor = torch.unique(out_tensor, dim=0)

    if not no_bar:
        print(
            f"[INFO]: (generate_comp_tensors) Generated {'unique_cnt ' if unique else ''}{out_tensor.shape[0]} tensors"
        )
...
def check_correct_gates(qc, num_of_qubits, gate_pool, max_gates, allowed_gate_clrs):
    tensor = data_con.encode_circuit(
        qc, num_of_qubits, data_con.gate_pool_to_gate_classes(gate_pool), max_gates
    )
    gen_gate_clrs = torch.unique(tensor.abs()).tolist()
    gate_corr = set(gen_gate_clrs).issubset(
        set(allowed_gate_clrs)
    )  # are gates correct?
    return gate_corr
...
def get_gate_and_U_acc(
    ...
    # -------------------------
    # decode
    qc_list, error_cnt = convert_tensors_to_circuits(
        out_tensor, gate_pool, params_tensor=params
    )
    if not no_bar:
        print(f"Error circuits: {error_cnt}")
    ...
    for qc in qc_list:
        ...
        gate_corr = check_correct_gates(
            qc, num_of_qubits, gate_pool, max_gates, allowed_gate_clrs
        )
        ...
        # comb_corr_qc.append(qc)
        if gate_corr and u_corr:
            comb_corr_qc.append(qc)
        if gate_corr:
            gate_corr_qc.append(qc)
        if u_corr:
            u_corr_qc.append(qc)
...
def test_comp_acc(
    ...
    for U, y in tqdm(zip(Us, ys), total=len(Us)):
        ...
        if len(qc_list) > 0:
            uniques_cnt = (
                torch.stack(
                    [
                        data_con.encode_circuit(
                            qc,
                            num_of_qubits,
                            data_con.gate_pool_to_gate_classes(gate_pool),
                            max_gates,
                        )
                        for qc in qc_list
                    ]
            ...
            num_found_distinct_circuits = (
                torch.stack(
                    [
                        ...
                        for qc in comb_corr_qc
                    ]
                )
                .unique(dim=0)
                .shape[0]
            )  # how many distinct exact solutions we have
        else:
    ...
    print(
        f"Solved {solved_tasks} correctly (at least one qc) that is {100 * solved_tasks / len(num_found_distinct_circuits_s):0.2f}%"
    )
...
def test_comp_acc_on_testset(
    ...
    if hasattr(test_dataset, "z"):  # mixed dataset has padding but a z record!
        Us, ys, zs = uniquify_tensor_dataset(
            test_dataset.U, test_dataset.y, test_dataset.z
        )
        ...
        Us, ys, xs = uniquify_tensor_dataset(
            test_dataset.U, test_dataset.y, test_dataset.x
        )
    ...
    if exists(fix_y):
        ys = [fix_y for y in ys]
    else:
        ys = [prompt_mod(y) for y in ys]
...
def test_comp_acc_on_rnd_samples(
    pipeline,
    samples,
    num_of_U,
    system_size,
    ...
    enc_t, ys, Us = data_con.gen_compilation_rndGates_dataset(
        num_of_U, num_of_qubits, rnd_min_gates, rnd_max_gates, gate_pool
    )
    task_qc_len_s = get_tensor_gate_length(
        enc_t
    )  # should give a complexity meassure, longer circuits have a more complex unitary to compile?
...
def plot_hist_overview(
    out_tuple,
    num_of_samples_per_U,
    rnd_min_gates,
    rnd_max_gates,
    max_gates,
    num_of_qubits,
    ...
    (
        ...
        uniques_cnt_s,
        error_cnt_s,
        num_found_circuits_s,
        task_qc_len_s,
    ) = out_tuple

    if not exists(rnd_min_gates):
        rnd_min_gates = ""
    if not exists(rnd_max_gates):
        rnd_max_gates = ""

    fig, axs = plt.subplots(
        2, 3, figsize=(13, 6.4), squeeze=False, constrained_layout=True
    )
    fig.suptitle(
        f"Histogram of compilation accuracies       (Unitary cnt={len(acc_s)}, samples_per_U={num_of_samples_per_U}  {rnd_min_gates=}  {rnd_max_gates=}  {max_gates=}  qubits={num_of_qubits})"
    )
